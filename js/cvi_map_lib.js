 ...
 * syntax:
	...
	cvi_map.defaultBordercolor 	= '#0000ff'; 	//STR '#333333'-'#ffffff'
	cvi_map.defaultAreacolor 	= '#333333'; 	//STR '#333333'-'#ffffff'
...
function performRGBColor(val,color){
	if(!val.match(/^#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]$/i)) {val = color||'#333333'; }
...
function extAreaOver(image,area) {
	...
	var ac = performRGBColor(img.options['areacolor'],'#333333');
...
var cvi_map = {
	...
	modify: function(image, options) {
		try {		
			...
			var acolor = performRGBColor(acolor,'#333333');	
